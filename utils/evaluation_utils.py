...
def draw_DOTA_image(imgsrcpath, imglabelspath, dstpath, extractclassname, thickness=None, extend='.png', colors=None, txt_color=(255, 255, 255), hide_labels=False):
    """
    ...
    num = len(filelist)
    for n, fullname in enumerate(filelist):  # fullname='/.../P000?.txt'
        print(f'draw_processing: {n+1}/{num}')
        objects = []
        ...
        img_fullname = os.path.join(imgsrcpath, name + extend)
        img_savename = os.path.join(dstpath, name + '_' + extend)
        ...
        for i, obj in enumerate(objects):
            ...
            cv2.drawContours(image=img,
                            ...
                            #  color=colors[int(extractclassname.index(classname))],
                             color=colors(int(extractclassname.index(classname)),True),
                             thickness=lw)
            if not hide_labels:
                conf = confs[i]
                label = '%s %.2f' % (classname, float(conf))
                tf = max(lw - 1, 1)  # font thicknes
                # xmax, xmin, ymax, ymin = max(poly[0::2]), min(poly[0::2]), max(poly[1::2]), min(poly[1::2])
                poly = np.array(poly)
                xmax, xmin, ymax, ymin = max(poly[:, 0]), min(poly[:, 0]), max(poly[:, 1]), min(poly[:, 1])
                x_label, y_label = int((xmax + xmin)/2), int((ymax + ymin)/2)
                w, h = cv2.getTextSize(label, 0, fontScale=lw / 3, thickness=tf)[0]  # text width, height
                cv2.rectangle(img,
                              (x_label, y_label),
                              (x_label + w + 1, y_label + int(1.5*h)),
                              colors(int(extractclassname.index(classname)),True), 
                              -1, cv2.LINE_AA)
                cv2.putText(img, label, (x_label, y_label + h), 0, lw / 3, txt_color, thickness=tf, lineType=cv2.LINE_AA)
