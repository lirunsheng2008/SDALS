...
def non_max_suppression_obb(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=1500, len_encode=7):
    ...
    """
    nc = prediction.shape[2] - 5 - len_encode  # number of classes
    xc = prediction[..., 4] > conf_thres  # candidates
    class_index = nc + 5
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        try:
            # train.py
            A = 2 * torch.logspace(1, len_encode, len_encode, base=a, device=x.device).half()
            B = (1 - a * (1 - pow(a, len_encode)) / (1 - a)) * torch.ones([x.shape[0], 1], device=x.device).half()
            encode_angle = torch.round(angle)
            theta_pred = 90 * (torch.matmul(encode_angle, A.unsqueeze(dim=1)) + B)
        except:
            # val.py
            A = 2 * torch.logspace(1, len_encode, len_encode, base=a, device=x.device).float()
            B = (1 - a * (1 - pow(a, len_encode)) / (1 - a)) * torch.ones([x.shape[0], 1], device=x.device).float()
            encode_angle = torch.round(angle)
            theta_pred = 90 * (torch.matmul(encode_angle, A.unsqueeze(dim=1)) + B)
